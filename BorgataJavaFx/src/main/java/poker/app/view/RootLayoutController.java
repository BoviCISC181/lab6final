				...
				for (final MenuItem mi : m.getItems()){
					mi.setOnAction(new EventHandler<ActionEvent>(){
						...
						public void handle(ActionEvent e){
							int ruleID = 0;
							for (GameRuleDomainModel g : GameRuleBLL.getRules()){
								if (g.getRULENAME() == mi.getText()){
									mainApp.setGRDM(g);
									ruleID = g.getRULEID();
								}
							}
							mainApp.setGRCDM(GameRuleCardsBLL.getCardsRules(ruleID));
							
//							ArrayList<GameRuleCardsDomainModel> grcdmList = new ArrayList<GameRuleCardsDomainModel>();
//							int[] cardslist = new int[4];
//							for (int i=0;i<4;i++){
//								cardslist[i] = grcdmList.get(i).getNBROFCARDS();
//							}
//							
							//PokerTableController.setRle(new Rule(mainApp.getGRDM(), cardslist));
							PokerTableController.setRle(new Rule(mainApp.getGRDM(), mainApp.getGRCDM()));
						}
					});
				}
			}
